...
class PublicMessage extends Connection
{
    ...
    public function selectMessage(){
        $stmt = $this->db->prepare("SELECT * FROM `tb_public_forwards` WHERE `send` = 0 AND `type` = 'users'");
        $stmt->execute();
        $result = $stmt->fetch();
        return $result;
    }
    public function getUsers($result1 , $bot){
        ...
        $bot->sendMessage($result1->chat_id, 'پیام شما با موفقیت به تمام اعضای ربات ارسال گردید.');
    }
}
